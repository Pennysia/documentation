...
Walletless lanes such as InstaSwap, RocketX CEX, and RocketX Private. The user transfers sell tokens to an ephemeral deposit address; fill is async off Settlement. Quote APIs return estimated output and deposit metadata rather than Settlement calldata. Contact Pennysia for onboarding.
...
| **Deposit-address** | InstaSwap / RocketX CEX / RocketX Private deposit → operator fill |
