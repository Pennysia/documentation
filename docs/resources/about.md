---
title: About
description: Roadmap, changelog, and project history.
lang: en-US
layout: doc
footer: true
lastUpdated: true
outline: [2, 3]
---

# About

## Roadmap

### Live now

- Swap app on Ethereum Mainnet at [app.pennysia.com](https://app.pennysia.com)
- 28 integrated exchanges (thousands of liquidity sources, 12,400+ tokens)
- Auto-fallback for failed routes
- Worst Acceptable Price protection
- Gas-aware quote ranking
- Settlement contract tested with automated security tools. See [Security](../get-started/security.md)
- Developer documentation and SDKs

### Coming soon

- More exchanges and liquidity sources
- Additional chain support
- Advanced routing features
- Mobile app

::: tip :book: TIP
Have a feature request? [Contact us](./contact.md).
:::

## Changelog

Notable product and documentation updates.

### September 2026

- RocketX added as the 27th and 28th integrated sources (Settlement SYNC calldata + walletless deposit-address)
- InstaSwap added as the 26th integrated exchange (deposit-address lane)
- Bebop BopAMM added as the 25th integrated exchange

### August 2026

- Swap app launched on Ethereum Mainnet at [app.pennysia.com](https://app.pennysia.com)
- 24 exchanges integrated with parallel quote fanout
- Pennysia Settlement deployed at `0x3Aad97E5a91b8e43b7Dc830aCEb004307678795E`. See [Deployments](../protocol/deployments.md)
- Auto Fallback, WAP, and gas-aware quote ranking live
- OpenOcean shown as **De1** in the app UI
- Developer docs published (contracts-sdk, backend-sdk, integration guide)
- Documentation site redesigned to match Pennysia brand guidelines

### Earlier

- Pennysia won 1st place at the [Sonic S-tier Global Hackathon](https://dorahacks.io/hackathon/sonic-s-tier/detail) (August–September 2025)

::: tip :book: TIP
For feature requests, use the [feedback form](https://docs.google.com/forms/d/e/1FAIpQLScnudg5yaAFL1ZOz28co1CJ202a3k2ntr3LhLa-rCohBQIorg/viewform) or [contact us](./contact.md).
:::

## Hackathon

Pennysia won **1st place** at the [Sonic S-tier Global Hackathon (August–September 2025)](https://dorahacks.io/hackathon/sonic-s-tier/detail), competing against hundreds of teams worldwide.

::: info :information_source: INFO
This page is an archive of where Pennysia started. The product has evolved significantly since the hackathon.
:::

### All Hackathon Winners

The announcement showcasing all winners is [here](https://x.com/SonicWorldHQ/status/1973693977116495939).

[![Winners](/Winners.png)](https://x.com/SonicWorldHQ/status/1973693977116495939)

### Pennysia 1st Place

The separate announcement for Pennysia winning 1st place is [here](https://x.com/PennysiaLabs/status/1973696631813136445).

[![PennysiaWon](/PennysiaWon.png)](https://x.com/PennysiaLabs/status/1973696631813136445)

<br/>

[![FirstPlace](/FirstPlace.png)](https://x.com/SonicWorldHQ/status/1973693995168731624)

### Project Submission

Our hackathon project submission on [Dorahacks](https://dorahacks.io/buidl/32575).

[![Dorahacks](/Dorahacks.png)](https://dorahacks.io/buidl/32575)

### Demo Video

A quick demo of the Pennysia web app:

<YouTubeEmbed video-id="IKanvcRU8FM" />
